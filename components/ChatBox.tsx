...
export default function ChatBox() {
  ...
  useEffect(() => {
    // 兼容旧版本：将旧的单会话 chat-history 迁移为多会话
    const oldHistory = localStorage.getItem('chat-history');
    const savedSessions = localStorage.getItem('chat-sessions');
    const savedActiveId = localStorage.getItem('chat-active-session-id');

    if (oldHistory && !savedSessions) {
      // 首次迁移：将旧聊天记录作为第一个会话
      const oldMessages = JSON.parse(oldHistory);
      const defaultId = generateId();
      const firstMsg = oldMessages.find((m: any) => m.role === 'user');
      const name = firstMsg
        ? firstMsg.content.trim().length > 25
          ? firstMsg.content.trim().slice(0, 25) + '...'
          : firstMsg.content.trim()
        : '新对话';

      setSessions([{ id: defaultId, name, createdAt: Date.now() }]);
      setActiveSessionId(defaultId);
      setMessages(oldMessages);
      localStorage.setItem(`chat-session-${defaultId}`, JSON.stringify(oldMessages));
      localStorage.removeItem('chat-history');
      return;
    }

    if (savedSessions) {
      const parsed: SessionMeta[] = JSON.parse(savedSessions);
      setSessions(parsed);
      ...
      if (targetId) {
        ...
        const saved = localStorage.getItem(`chat-session-${targetId}`);
        if (saved) setMessages(JSON.parse(saved));
      }
    } else {
      // 首次使用，创建默认会话
      const defaultId = generateId();
      setSessions([{ id: defaultId, name: '新对话', createdAt: Date.now() }]);
      setActiveSessionId(defaultId);
      setMessages([]);
    }
  }, []);
  ...
  useEffect(() => {
    if (activeSessionId && messages.length > 0) {
      localStorage.setItem(`chat-session-${activeSessionId}`, JSON.stringify(messages));
    }
  ...
  useEffect(() => {
    ...
    setSessions((prev) =>
      prev.map((s) => {
        if (s.id === activeSessionId && s.name === '新对话') {
  ...
  const handleNewSession = () => {
    ...
    const newSession: SessionMeta = { id: newId, name: '新对话', createdAt: Date.now() };
  ...
  const handleDeleteSession = (sessionId: string) => {
    ...
    if (hasMessages && !confirm('确定要删除此对话吗？')) return;
    ...
    if (remaining.length === 0) {
      // 删光了，创建一个新的默认会话
      const newId = generateId();
      setSessions([{ id: newId, name: '新对话', createdAt: Date.now() }]);
      setActiveSessionId(newId);
      setMessages([]);
    } else {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessages([
        ...newMessages,
        { role: 'assistant', content: '⚠️ 网络请求失败，请检查网络连接后重试。' },
      ]);
      return;
    }
    ...
    if (!response.ok) {
      setMessages([
        ...newMessages,
        { role: 'assistant', content: `⚠️ 服务器错误 (${response.status})，请稍后重试。` },
      ]);
      return;
    }
    ...
    try {
      ...
      if (aiContent) {
        ...
        setMessages([
          ...newMessages,
          { role: 'assistant', content: '⚠️ 读取响应失败，请重试。' },
        ]);
      }
    }
  };
  ...
  return (
    <div className="flex h-dvh bg-gradient-to-br from-slate-900 via-purple-950 to-indigo-950 overflow-hidden">
      {/* 背景动态光晕 */}
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          />
          {/* 侧边栏 */}
          <div className="absolute left-0 top-0 h-dvh animate-slide-in">
            <SessionList
              sessions={sessions}
              activeSessionId={activeSessionId}
              onNewSession={() => { handleNewSession(); setSidebarOpen(false); }}
              onDeleteSession={handleDeleteSession}
              onSwitchSession={(id) => { handleSwitchSession(id); setSidebarOpen(false); }}
            />
          </div>
        </div>
      )}
      ...
      <div className="relative z-10 flex-1 flex flex-col min-w-0 h-dvh">
        <div className="flex-1 max-w-4xl mx-auto w-full p-4 flex flex-col gap-4 min-h-0">
          {/* ===== 顶部工具栏（汉堡菜单 + 上传按钮） ===== */}
          <div className="shrink-0 backdrop-blur-2xl bg-white/[0.06] border border-white/[0.08] rounded-2xl p-3 shadow-[0_8px_32px_rgba(0,0,0,0.4)] flex items-center gap-3">
            ...
            <button
              ...
              className="md:hidden group flex items-center justify-center w-9 h-9 rounded-xl text-white/60 bg-white/[0.04] border border-white/[0.08] hover:bg-white/[0.1] hover:text-white transition-all duration-300 shrink-0"
            >
            ...
            <div className="md:hidden w-px h-5 bg-white/[0.08]" />
          ...
          <div className="flex-1 overflow-y-auto backdrop-blur-2xl bg-white/[0.04] border border-white/[0.08] rounded-2xl p-4 shadow-[0_8px_32px_rgba(0,0,0,0.3)]">
            {messages.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center text-white/25 select-none">
                <svg
                  className="w-20 h-20 mb-5 opacity-40"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  strokeWidth={0.8}
            ...
            {messages.map((msg, index) => (
              <div
                key={index}
                className={`flex mb-4 ${msg.role === 'user' ? 'justify-end' : 'justify-start'} last:mb-0`}
              >
                    ...
                    {msg.role === 'user' ? (
                      ...
                      <ReactMarkdown
                        ...
                        components={{
                          ...
                          h1({ children }) {
                            return (
                              <h1 className="text-lg font-bold mt-4 mb-2 text-white border-b border-white/[0.06] pb-1">
                                {children}
                              </h1>
                            );
                          },
                          h2({ children }) {
                            return (
                              <h2 className="text-base font-bold mt-3 mb-2 text-white/95">
                                {children}
                              </h2>
                            );
                          },
                          h3({ children }) {
                            return (
                              <h3 className="text-sm font-semibold mt-3 mb-1 text-white/90">
                                {children}
                              </h3>
                            );
                          },
                          p({ children }) {
                            return (
                              <p className="my-1.5 text-white/85 leading-relaxed">
                                {children}
                              </p>
                            );
                          },
